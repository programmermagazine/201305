...
影像從 gif 轉為 jpg 的方法，使用 imagemagic (command line)，參考：http://www.imagemagick.org/script/convert.php
*/
...
/*  child_process.exec('mimetex -d "'+tex+'" > '+filename+".gif", function(err, stdout, stderr) {
    log("mimetex generate tex:%s file:%s.gif finished!\n", tex, filename);
    child_process.exec('convert '+filename+'.gif '+filename+'.jpg', function(err, stdout, stderr) {
      log("convert file:%s.gif to %s.jpg finished!\n", filename, filename); 
    });
  });
*/  
}
